...
#         rewritten_query = response.content if response and response.content else user_input
#     except Exception as e:
#         print(f"Error in rewritten_query_agent: {e}")
#         rewritten_query = user_input  # fall back to original query

#     return {"rewritten_query": rewritten_query}


# def retriever_agent(state: State) -> dict:
#     # Fall back to original query if rewrite produced nothing
#     rewritten_query = state.get("rewritten_query") or state.get("query") or ""

#     if not rewritten_query.strip():
#         print("Warning: empty query passed to retriever — returning no results.")
#         return {"content": []}
...
#     try:
#         response = llm.invoke(messages)
#         answer = response.content if response and response.content else "Sorry, I could not generate a response."
#     except Exception as e:
